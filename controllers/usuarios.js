  ...
  var UsuarioController = {
      ...
      post: function(req, res) {
        if (validacao(req, res)) {
          ...
          /* Verificar E-mail Unique */
          Usuario.findOne({'email': model.email}, function(err, data) {
            if (data) {
              req.flash('erro', 'O E-mail informado já encontrasse cadastrado em nosso sistema, tente outro');
              res.render('usuarios/create', {
                user: model
              });
            } else {
    ...
    delete: function(req, res) {
      Usuario.remove({
        ...
        if (err) {
          req.flash('erro', 'Erro ao excluir o usuário: ' + err);
          res.redirect('/usuarios');
        } else {
